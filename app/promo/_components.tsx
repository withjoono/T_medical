    ...
    <section className="grain relative isolate overflow-hidden">
      ...
      {stats && stats.length > 0 && (
        <div className="relative border-t border-white/10">
          <div
            className={`mx-auto grid max-w-5xl grid-cols-2 ${
              stats.length === 2
                ? "sm:grid-cols-2"
                : stats.length === 3
                  ? "sm:grid-cols-3"
...
export function PromoSection({
  ...
  return (
    <section
      className={`border-t border-hair px-6 py-20 sm:px-12 sm:py-28 ${
...
export function FeatureGrid({
  ...
  return (
    <div
      className={`grid overflow-hidden border-l border-t border-hair bg-paper-50 ${cols}`}
    >
      {items.map((f, i) => {
        const Icon = f.icon;
        return (
      ...
      {items.map((c) => {
        ...
        return (
          <Link
            key={c.href + c.title}
            href={c.href}
            className="group relative flex flex-col border-b border-r border-hair p-8 transition-colors duration-300 hover:bg-white"
          >
            <span className="absolute inset-x-0 top-0 h-px origin-left scale-x-0 bg-jade-600 transition-transform duration-500 group-hover:scale-x-100" />
